...
def rightClick(event):
	...
	for dx in range(x_min,x_max):
		for dy in range(y_min,y_max):
			blankPointS.append(list(miniMat[dy][dx]))

	canvas2.create_rectangle(x_max*scaleFactor, y_max*scaleFactor, x_min*scaleFactor, y_min*scaleFactor, outline="red", dash = (3,5), width = 5)
	if len(leafPointS) and len(blankPointS):learn2()
...
def finish_subwin(event):
	...
	x_min, x_max = min(tempxList)+start_x, max(tempxList)+start_x
	y_min, y_max = min(tempyList)+start_y, max(tempyList)+start_y
...
def settingFinished(event):
	...
	canvas2.bind("<Button 1>",leftClick)
	canvas2.bind("<Button 3>",rightClick)
...
def fatality(event):
	...
	global leafInfoS
	global miniMat
	newCordinateS = fatal_go(cordinateS)
	for [ny, nx] in newCordinateS:
		(canvas2.create_oval(nx*scaleFactor, ny*scaleFactor, nx*scaleFactor, ny*scaleFactor, width = 4, fill = 'green', outline="green"))
	newCordinateS = fatal_go(newCordinateS)
	for [ny, nx] in newCordinateS:
		(canvas2.create_oval(nx*scaleFactor, ny*scaleFactor, nx*scaleFactor, ny*scaleFactor, width = 4, fill = 'red', outline="red"))
...
if 1:
	...
	from ImageOnCanvas import *

	activateCanvas(visualShock)