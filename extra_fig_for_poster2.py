if 1:
	...
	lumSeries = []
	rgbSeries = []
	for cybers in dataStorm:
		lumSeries.append(np.nansum(cybers[1]))
		rgbSeries.append(np.nansum(cybers[2][:,:,0]))
	rgbSeries = monotonical(rgbSeries)		
	rgbSeries = savgol_filter(rgbSeries, 51, 3)
	lumSeries = savgol_filter(lumSeries, 51, 3)
	rgbSeries = regular(rgbSeries[20:])
	lumSeries = regular(lumSeries[20:])
	plt.rcParams['axes.facecolor'] = 'black'
	#plt.subplot(1,1,1,axisbg='black')
	#plt.plot(timeList[20:],rgbSeries,'white',linewidth=5)
	plt.plot(timeList[20:],lumSeries,'white',linewidth=5)
	plt.show()
