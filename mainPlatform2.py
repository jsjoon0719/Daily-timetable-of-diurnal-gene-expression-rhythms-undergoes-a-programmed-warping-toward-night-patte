...
def sorter(series):
	...
	return regularize(result), start
...
def compare(gene):
	...
	sYoung, sy = sorter(youngSeries)
	sOld, so = sorter(oldSereis)
	return sy,so
...
with open('peak2.txt','w') as F:
	for n in range(len(youngDB)):
		F.write(vGeneS[n]+'\t'+ str(youngDB[n])+'\t'+str(oldDB[n])+'\tX\n')
EXIT()
