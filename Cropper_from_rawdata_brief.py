...
def finish_subwin():
	...
	(canvas.create_text(start_x,start_y, font='Times 20 bold', text = 'Marker# '+str(leafN), fill = 'yellow'))
	leafInfoS.append([leafN,(x_min-1,y_min-1,x_max+2,y_max+2)])
...
def settingFinished(event):
	...
	global sub_x, sub_y, k1, size_X, size_Y
...
def save(event):
	...
	for [leafN, (start_x,start_y,end_x,end_y)] in leafInfoS:
		...
		for file in fileList:
			if random.random() > 0.9:print ('Reading files...', file, '#', leafN)
			...
			with Image.open(folder+'/'+file) as img:
				...
				if TAE < 0:
					...
					continue
				tag = tag + '@' + str(TAE)
				img2 = np.median((np.array(img)[start_y:end_y,start_x:end_x]))
				if len(file.split('_')) > 2:
		...
		for lumtag in dataDict.keys():
			...
			dataStrom.append([TAE,lumImage,RGBs])
		print (dataStrom)
		try:os.mkdir('temp/'+folder)
		except:pass
		dump(dataStrom, 'temp/'+folder+'/brief_leaf_'+str(leafN))
		saveNow()
		print ('Complete!')
...
if 1:
	...
	root.bind('<Button 3>', settingFinished)
	root.bind('<R>',save)
	root.bind('<r>',save)
	root.mainloop()