...
def getPeriodFB(downPeakS,upPeakS,TimeList, minTime = 5, minGap = 4):
	firstPeak = 0
	firstPeak_time = 0
	though = 0
	though_time = 0
...
def regular(series):
	...
	return result
def smoothen(Curve,win=19,order=3):
	return savgol_filter(Curve, win, order)
def derivative(Curve):
	temp = list(Curve[1:len(Curve)]-Curve[0:len(Curve)-1])
	temp.append(temp[-1])
	return temp
def findPeakThough(Curve, TimeList, gap_required = 24):
